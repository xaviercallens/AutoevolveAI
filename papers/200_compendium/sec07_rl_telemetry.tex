...
\midrule
Training Set ($N = 140$) & 0.6944 & 0.0205 & \textbf{-97.05\%} & $-0.0250 \to +57.2886$ \\
Held-Out Validation ($N = 60$) & 0.6941 & 0.0063 & \textbf{-99.10\%} & $-0.0184 \to +54.7025$ \\
\midrule
\textbf{Generalization Gap} & \multicolumn{4}{c}{\textbf{-0.0142} (no overfitting; consistent out-of-distribution margin expansion)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Training Set ($N = 140$) & 0.1714 & 0.0082 & \textbf{-95.22\%} \\
Held-Out Validation ($N = 60$) & 0.1707 & 0.0116 & \textbf{-93.20\%} \\
\midrule
\textbf{Generalization Gap} & \multicolumn{3}{c}{\textbf{0.0034} (stable non-collapsing representation with $\tau = 0.05$)} \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
    \item \textbf{Monotonic Energy Descent Criterion:} Across all 197 valid tasks, the learned policy yields candidate solutions with mean energy delta $\Delta E = E_{\text{chosen}} - E_{\text{rejected}} = \mathbf{-53.2092} < 0$.
