...
\midrule
Training Set ($N = 140$) & 0.6915 & 0.0141 & \textbf{-97.96\%} & $0.0338 \to +72.6444$ \\
Held-Out Validation ($N = 60$) & 0.6915 & 0.0004 & \textbf{-99.94\%} & $0.0322 \to +77.9176$ \\
\midrule
\textbf{Generalization Gap} & \multicolumn{4}{c}{\textbf{-0.0137} (no overfitting; consistent out-of-distribution margin expansion)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Training Set ($N = 140$) & 0.1668 & 0.0086 & \textbf{-94.84\%} \\
Held-Out Validation ($N = 60$) & 0.1713 & 0.0132 & \textbf{-92.28\%} \\
\midrule
\textbf{Generalization Gap} & \multicolumn{3}{c}{\textbf{0.0047} (stable non-collapsing representation with $\tau = 0.05$)} \\
\bottomrule
\end{tabular}
\end{table}

\begin{itemize}
    \item \textbf{Monotonic Energy Descent Criterion:} Across all 197 valid tasks, the learned policy yields candidate solutions with mean energy delta $\Delta E = E_{\text{chosen}} - E_{\text{rejected}} = \mathbf{-75.0042} < 0$.
