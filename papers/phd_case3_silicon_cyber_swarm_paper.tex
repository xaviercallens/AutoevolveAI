...
\caption{(a) Gate-level topological timing path for 4$\times$4 systolic array ($T_{\text{crit}} = 1.082\text{ ns}$, Slack $= +0.168\text{ ns}$); (b) Real POSIX \texttt{SCM\_RIGHTS} live socket descriptor migration completed in 1152.9~$\mu$s with zero dropped packets and negative RSS memory delta ($\Delta = -228.2\text{ MB}$).}
\label{fig:case3}
\end{figure}
...
\midrule
Silicon Architect & Tier 2 (GPT-4o) & 0.00e+00 & 0.00 & 2.80 & \texttt{66c2239c} \\
Cyber-Red Adversary & Tier 2 (Qwen2.5-Coder) & 0.00e+00 & 0.00 & 2.10 & \texttt{fa400f11} \\
Blue-Hardener Hypervisor & Tier 1 (Claude 3.5 Sonnet) & 0.00e+00 & 1.15 & 2.90 & \texttt{0afb5c9e} \\
\midrule
\textbf{Swarm Aggregate} & \textbf{Overall Gate: PASS} & \textbf{0.00e+00} & \textbf{63.37} & \textbf{2.90} & \texttt{0435ed1a} \\
\bottomrule
\end{tabular}}
\end{table}
...
\item Real migration latency: $t_{\text{migrate}} = 1152.9\text{ }\mu\text{s}$.
\item Resident memory reduction: $\Delta\text{RSS} = -228.2\text{ MB} < 0$.
\end{itemize}
