...
with open("tests/phase1/test_main.py") as f:
    content = f.read()
...
lines = content.split("\n")
new_lines = []
skip_decorator = "@pytest.mark.skipif(shutil.which('docker') is None, reason=\"Requires docker\")"
...
with open("tests/phase1/test_main.py", "w") as f:
    f.write("\n".join(new_lines))
