...
def generate_proposal(model: str, seed: int, prompt: str, system_prompt: str) -> tuple[str, int]:
    ...
    reply, record = APIExtractor(config=cfg.model, seed=seed).extract(prompt, system_prompt=system_prompt)
...
class Bench:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.results: dict[str, Any] = {
            ...
            "registry_root": str(self.registry_root.relative_to(ROOT)) if self.registry_root.is_relative_to(ROOT)
    ...
    def hypervisor(self, uc: str) -> AutopoiesisHypervisor:
        ...
        return AutopoiesisHypervisor(registry, sandbox=self.sandbox, rule=self.rule, sandbox_config=self.cfg.sandbox)

    def oracle(self, hv: AutopoiesisHypervisor, component: dict[str, Any], candidate_code: str) -> dict[str, Any]:
        """
        ...
        """
        inputs = eval(component["fuzz"], {"rng": random.Random(self.args.fuzz_seed)})  # trusted suite expression
        outcome = hv.differential_test(component["parent"], candidate_code, component["entry"], inputs)
        return {
    ...
    def uc1_better_child_promoted(self) -> None:
        ...
        for c in self.components:
            ...
            row.update(self.oracle(hv, c, hv.registry.active_code(c["name"])))  # audit what is live now
            rows.append(row)
            print(f"  UC1 {c['name']:<18} promoted={d.promoted} speedup={row['speedup']} "
                  f"E {row['parent_energy']} -> {row['child_energy']} oracle_mismatches={row['oracle_mismatches']}/"
                  f"{row['oracle_inputs']} ({d.reason})", flush=True)
        ...
        self.results["uc1"] = {
            ...
            "energy_parent_to_child": {r["component"]: round(r["child_energy"] / r["parent_energy"], 3)
                                       for r in rows if r["parent_energy"]},
            "rows": rows,
        }
        self.checkpoint()
    ...
    def uc2_fast_but_wrong_rejected(self) -> None:
        ...
        for c in self.components:
            ...
            measured = len(parents) == len(children) == self.rule.samples and all(s.valid for s in parents + children)
            verdict = judge_domination([s.energy for s in parents], [s.energy for s in children], self.rule) if measured else None
            rows.append({
                "component": c["name"],
                "new_pipeline_promoted": d.promoted,
                "rejected_at": d.stage,
                "tests_passed": f"{d.equivalence.child_passed}/{d.equivalence.total}",
                "first_failure": d.equivalence.child_failures[0] if d.equivalence.child_failures else "",
                ...
                "wrong_child_energy_is_lower": bool(verdict and verdict.child_median < verdict.parent_median),
                ...
                "benchmark_output_matches_parent": {s.output for s in children} == {s.output for s in parents},
            ...
            r = rows[-1]
            print(f"  UC2 {c['name']:<18} new={'PROMOTED' if d.promoted else 'rejected@' + d.stage} "
                  f"tests={r['tests_passed']} E {r['parent_energy']} -> {r['wrong_child_energy']} "
                  f"legacy_would_swap={r['legacy_rule_would_swap']}", flush=True)
        self.results["uc2"] = {
            ...
            "rejected_at_equivalence_gate": sum(r["rejected_at"] == "equivalence" and not r["new_pipeline_promoted"] for r in rows),
            ...
            "accepted_by_domination_without_gate": sum(r["domination_alone_would_swap"] for r in rows),
            "undetectable_from_benchmark_output": sum(r["benchmark_output_matches_parent"] for r in rows),
    ...
    def uc3_aa_noise(self) -> None:
        ...
        for repeat in range(1, self.args.aa_repeats + 1):
            for c in self.components:
                ...
                row.update({
                    ...
                    "energy_spread_max_over_min": round(max(s.energy for s in d.parent_samples + d.child_samples)
                                                        / min(s.energy for s in d.parent_samples + d.child_samples), 2) if pairs else None,
                })
                rows.append(row)
                print(f"  UC3 {c['name']:<18} #{repeat} new_rule_promoted={d.promoted} wins={row['pair_wins']} "
                      f"legacy_first_sample_swap={row['false_promotion_legacy_rule']} legacy_all={row['legacy_swaps_over_all_pairs']}", flush=True)
        n = len(rows)
        ...
        self.results["uc3"] = {
            ...
            "false_promotion_rate_new_rule": round(sum(r["false_promotion_new_rule"] for r in rows) / n, 3) if n else 0.0,
            "false_promotion_rate_legacy_rule": round(sum(r["false_promotion_legacy_rule"] for r in rows) / n, 3) if n else 0.0,
            "legacy_single_sample_comparisons": legacy_den,
            "legacy_single_sample_swaps": legacy_num,
            "median_energy_spread_max_over_min": round(statistics.median(spreads), 2) if spreads else 0.0,
            "rows": rows,
        }
        self.checkpoint()
    ...
    def uc4_rollback(self) -> None:
        ...
        for c in self.components:
            ...
            row: dict[str, Any] = {"component": name, "promoted": d.promoted, "version_after_promote": reg.active_version(name)}
            if d.promoted:
                ...
                row.update({
                    ...
                    "probe_result_correct": getattr(live, c["entry"])(*c["probe_args"]) == c["probe_expected"],
                    "lineage_events": " > ".join(events),
                    "rollback_logged": last["event"] == "rollback" and last["from_version"] == d.child_version and last["to_version"] == 1,
                    "child_version_file_kept": reg.version_path(name, int(d.child_version or 0)).exists(),
                })
                try:
                    hv.rollback(name)
                ...
                row["restored"] = bool(
                    restored == 1 and row["active_hash_is_parent"] and row["active_code_is_parent"]
                    and row["live_file_after_rollback"] == "v0001.py" and row["probe_result_correct"]
                    and row["rollback_logged"] and row["child_version_file_kept"] and row["second_rollback_refused"]
                    and row["corrupted_pointer_recovered_to"] == 1 and row["repair_logged"]
                )
            else:
                row.update({"restored": False, "reason": d.reason})
            rows.append(row)
            print(f"  UC4 {name:<18} promoted={d.promoted} restored={row['restored']} "
                  f"lineage={row.get('lineage_events', '-')}", flush=True)
        self.results["uc4"] = {
            "title": "Rollback",
    ...
    def uc5_llm_evolution(self) -> None:
        ...
        for c in self.components[: self.args.uc5_components]:
            ...
            for proposal in range(1, self.args.uc5_proposals + 1):
                t0 = time.time()
                reply, embedding_dim = llm.submit(generate_proposal, self.args.model, proposal,
                                                  prompt + retry_suffix, LLM_SYSTEM_PROMPT).result()
                self.generations += 1
                row: dict[str, Any] = {"component": c["name"], "proposal": proposal, "seed": proposal,
                                       "generation_seconds": round(time.time() - t0, 1),
                                       "embedding_dim": embedding_dim}
                try:
                    code = extract_code(reply).code + "\n"
                except NoCodeFoundError:
                    row.update({"code_extracted": False, "gate_says_correct": False, "oracle_agrees": False, "promoted": False,
                                "decided_at": "extraction", "reason": "no code block in the reply", "code": reply[:600]})
                    rows.append(row)
                    retry_suffix = LLM_RETRY_SUFFIX.format(code="", reason=row["reason"], failures="")
                    continue
                d = hv.evolve(c["name"], code, c["tests"], c["workload"])
                row.update(self.decision_row(d))
                row.update({"code_extracted": True, "gate_says_correct": d.equivalence.eligible and d.stage != "benchmark",
                            "first_failure": d.equivalence.child_failures[0] if d.equivalence.child_failures else "",
                            "code": code})
                ...
                rows.append(row)
                print(f"  UC5 {c['name']:<18} proposal {proposal}: tests={row['tests_passed']} promoted={d.promoted} "
                      f"speedup={row['speedup']} oracle_mismatches={row['oracle_mismatches']}/{row['oracle_inputs']} "
                      f"({d.stage}: {d.reason})", flush=True)
                self.checkpoint_partial("uc5", rows)
                if d.promoted:
                    break
                failures = "".join(f"- {f}\n" for f in d.equivalence.child_failures)
                retry_suffix = LLM_RETRY_SUFFIX.format(code=code, reason=d.reason, failures=failures)
        llm.shutdown()
        ...
        self.results["uc5"] = {
            ...
            "gate_passed_but_oracle_found_wrong": sum(r["gate_says_correct"] and not r["oracle_agrees"] for r in rows),
            "gate_rejected_but_oracle_found_correct": sum(r["code_extracted"] and not r["gate_says_correct"] and r["oracle_agrees"] for r in rows),
            "promoted": len(promoted),
            "incorrect_promoted": sum(r["promoted"] and not (r["oracle_agrees"] and r["audited_live_version"]) for r in rows),
    ...
    def gate(self) -> bool:
        ...
        if "uc1" in r:
            u = r["uc1"]
            checks["G1 every correct faster child is promoted, becomes the active version, and agrees with the parent on the held-out oracle"] = (
                u["components"] > 0 and u["promoted"] == u["active_is_child"] == u["components"]
                and u["live_versions_agreeing_with_held_out_oracle"] == u["components"]
            )
        if "uc2" in r:
            u = r["uc2"]
            checks["G2 every fast-but-wrong child is rejected at the equivalence gate, while the legacy rule would swap at least one in"] = (
                u["wrong_children"] > 0 and u["rejected_at_equivalence_gate"] == u["parent_still_active"] == u["wrong_children"]
                and u["accepted_by_legacy_rule"] > 0
            )
        if "uc3" in r:
            u = r["uc3"]
            checks["G3 zero false promotions of identical code over >= 10 A/A trials under the new rule"] = (
                u["trials"] >= 10 and u["false_promotions_new_rule"] == 0
            )
        if "uc4" in r:
            u = r["uc4"]
            checks["G4 rollback restores the parent's version, hash and behaviour and is logged, for every component"] = (
                u["components"] > 0 and u["fully_restored"] == u["components"]
            )
        if "uc5" in r:
            u = r["uc5"]
            checks["G5 the LLM lands at least one promoted improvement, and the held-out differential oracle finds no promoted version that differs from the original parent"] = (
...
def main() -> int:
    ...
    parser.add_argument("--samples", type=int, default=DominationRule().samples, help="Interleaved pairs per comparison")
    ...
    parser.add_argument("--fuzz-seed", type=int, default=20260921, help="Seed of the held-out oracle's inputs")
    parser.add_argument("--limit", type=int, default=0, help="Only the first N components (smoke run)")
    parser.add_argument("--uc", nargs="+", default=["1", "2", "3", "4", "5"], help="Use cases to run, in order")
    parser.add_argument("--resume", action="store_true", help="Keep finished use cases from an interrupted run's results.json")
    parser.add_argument("--out", default=str(ROOT / "results" / "phase3_evolution"))
    args = parser.parse_args()

    bench = Bench(args)
    steps = {"1": bench.uc1_better_child_promoted, "2": bench.uc2_fast_but_wrong_rejected, "3": bench.uc3_aa_noise,
             "4": bench.uc4_rollback, "5": bench.uc5_llm_evolution}
    for uc in args.uc:
        print(f"\n=== UC{uc} ===", flush=True)
        t0 = time.time()
        steps[uc]()
