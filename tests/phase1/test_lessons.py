...
ROTATE_STR = "Write a function `rotate_string(s, k)` that rotates a string to the left by k positions."
...
def test_retrieve_returns_only_lessons_above_threshold_best_first(tmp_path):
    ...
    memory.add(Lesson(task=ROTATE, code="def rotate_list(lst, k): return lst", failure="ZeroDivisionError"))
...
def test_format_lessons_includes_mistake_and_code_and_is_empty_without_hits():
    block = format_lessons([(0.5, Lesson(task=ROTATE, code="def rotate_list(): ...", failure="ZeroDivisionError on []"))])
