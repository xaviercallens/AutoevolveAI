    ...
    for t in range(n_tasks):
        for iteration, energy in ((1, 60.0), (2, 0.0), (1, 0.0)):
            ...
            lines.append(
                json.dumps(
                    {
                        "task": f"Write function number {t}",
                        "iteration": iteration,
                        "energy": energy,
                        "code": f"def f{t}():\n    return {len(lines)}",
...
class TestInsufficientData:
    def test_too_few_traces_fails_with_a_clear_message_and_writes_nothing(
        self, tmp_path: Path
    ) -> None:
        traces = _traces(tmp_path / "few.jsonl", n_tasks=2)
        with pytest.raises(
            runner.InsufficientDataError,
            match=r"only 6 usable verified traces over 2 tasks \(need >= 30",
        ):
    ...
    def test_self_graded_traces_do_not_count_as_usable(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(
            runner.InsufficientDataError, match="only 0 usable verified traces"
        ) as excinfo:
...
) -> tuple[dict[str, Any], bool, runner.Bench]:
    tmp_path = tmp_path_factory.mktemp("phase2_runner")
    bench = runner.Bench(_args(tmp_path, [_traces(tmp_path / "traces.jsonl", n_tasks=6)]))
    bench.train_all()
    for step in (
        bench.uc1_beats_constant,
        bench.uc2_discrimination,
        bench.uc3_pairs_ablation,
        bench.uc4_selection,
        bench.uc5_robustness,
    ):
        step()
    passed = bench.gate()
...
class TestResultsContract:
    ...
    def test_every_use_case_is_flat_with_rows(
        ...
        for key in ("uc1", "uc2", "uc3", "uc4", "uc5"):
            ...
            for row in uc["rows"]:
                assert all(isinstance(v, scalar) for v in row.values()), (
                    f"{key} row is not flat: {row}"
                )
            for name, value in uc.items():
                if name != "rows" and isinstance(value, dict):
                    assert all(isinstance(v, scalar) for v in value.values()), (
                        f"{key}.{name} nests too deep"
                    )
    ...
    def test_reported_numbers_are_recomputable_from_rows(
        self, finished_run: tuple[dict[str, Any], bool, runner.Bench]
    ) -> None:
        results, _, _ = finished_run
        uc1, uc2, uc4 = results["uc1"], results["uc2"], results["uc4"]

        assert uc1["mae_jepa"] == pytest.approx(
            sum(r["mae_jepa"] for r in uc1["rows"]) / 2, abs=1e-3
        )
    ...
    def test_uc3_refuses_to_conclude_on_too_few_transitions(
        self, finished_run: tuple[dict[str, Any], bool, runner.Bench]
    ) -> None:
        results, _, _ = finished_run
        uc3 = results["uc3"]
    ...
    def test_uc5_checks_all_ran_against_real_behaviour(
        self, finished_run: tuple[dict[str, Any], bool, runner.Bench]
    ) -> None:
        results, _, _ = finished_run
        ...
        assert set(rows) == {
            "no_representation_collapse",
            "dimension_mismatch_raises",
            "nan_and_empty_states_skipped",
            "predictions_within_0_100",
            "raw_embedding_predicts_like_training_input",
        }
        assert rows["raw_embedding_predicts_like_training_input"]["passed"] is True
        assert rows["raw_embedding_predicts_like_training_input"]["raw_norm_min"] > 1.5, (
            "fixture states are not unit vectors"
        )
        ...
        assert (
            0.0
            <= rows["predictions_within_0_100"]["min_prediction"]
            <= rows["predictions_within_0_100"]["max_prediction"]
            <= 100.0
        )
...
class TestLeakageAndReplication:
    ...
    def test_duplicates_cannot_satisfy_the_minimum_trace_guard(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(
            runner.InsufficientDataError, match="only 6 usable verified traces"
        ) as excinfo:
